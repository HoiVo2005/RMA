...
import { Newspaper, CheckCircle2, FileEdit, Radio, Trophy, Users, RefreshCcw } from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  async function runIngest() {
    ...
    if (res.error) {
      ...
      setResult(`Xong: quét ${d.sources} nguồn, tìm ${d.found}, thêm mới ${d.inserted}, bỏ qua ${d.skipped}, lỗi ${d.failed}.`);
    }
    load();
  }
    ...
    <AdminShell title="Tổng quan">
      ...
      <div className="panel" style={{ marginBottom: 24 }}>
        <div className="panel-head">
          ...
          <button className="btn btn-gold" onClick={runIngest} disabled={running}>
            {running ? <span className="spinner spinner-dark" /> : <RefreshCcw size={15} />}
            {running ? 'Đang lấy tin...' : 'Lấy tin ngay'}
          </button>
        </div>
        <div style={{ padding: 20 }}>
          <p style={{ margin: '0 0 10px', fontSize: 13.5, color: 'var(--ink-500)' }}>
            Quét toàn bộ nguồn tin đã bật RSS, lọc bài liên quan Real Madrid, tải nội dung + ảnh từ trang gốc và dịch
            sang tiếng Việt bằng AI. Bài mới sẽ được xuất bản tự động. Bạn cũng có thể lấy 1 bài viết bất kỳ theo liên
            kết trong trang <b>Bài viết</b>.
          </p>
              ...
              {stats.logs.map((l: any) => (
                <div className="log-item" key={l.id}>
                  <span style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span
                      className={`dot ${
                        l.status === 'completed' ? 'log-ok' : l.status === 'running' ? 'log-run' : 'log-warn'
                      }`}
                    />
                    {new Date(l.started_at).toLocaleString('vi-VN')}
                  </span>
                  <span>
                    Tìm {l.total_found} · Thêm {l.total_inserted} · Bỏ qua {l.total_skipped} · Lỗi {l.total_failed}
                  </span>
                </div>
              ))}
            </div>
